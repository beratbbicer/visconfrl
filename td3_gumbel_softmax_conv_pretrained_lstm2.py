...
class CriticNetwork(nn.Module):
    def __init__(self, action_dim, path, num_layers, bidirectional, freeze=True):
        ...
        self.output_layer1 = nn.Sequential(
            nn.Linear(self.hidden_dim // 2 + self.action_dim, self.hidden_dim // 4),
        ...
        # LSTM Layer
        # ====================================================================================
        self.rnn1 = nn.LSTM(input_size=self.hidden_dim, hidden_size=self.hidden_dim//2, num_layers=num_layers, batch_first=True, bidirectional=bidirectional)
        self.rnn2 = nn.LSTM(input_size=self.hidden_dim, hidden_size=self.hidden_dim//2, num_layers=num_layers, batch_first=True, bidirectional=bidirectional)
    ...
    def forward(self, state, action, h0, c0):
        ...
        q1 = self.projection1(self.forward_pass(x_q1, self.layers1, self.rnn1, h0, c0))
        q2 = self.projection2(self.forward_pass(x_q2, self.layers2, self.rnn2, h0, c0))
...
class PolicyNetwork(nn.Module):
    def __init__(self, action_dim, path, num_layers, bidirectional, freeze=True):
        ...
        # ====================================================================================
        self.rnn = nn.LSTM(input_size=self.hidden_dim, hidden_size=self.hidden_dim//2, num_layers=num_layers, batch_first=True, bidirectional=bidirectional)
    ...
    def forward(self, state, h0, c0):
        ...
        '''
        out, (h0_out, c0_out) = self.rnn(out, (h0, c0))
        out = self.projection(out.view(b,-1))
        # ================================================================
        # Output Layer
...
class AnnealedGumbelSoftmaxPolicy(nn.Module):
    def __init__(self, action_dim, weightspath, num_layers, bidirectional, freeze, init_temperature=1.0, min_temperature=0.1, max_temperature=1.0, anneal_rate=1e-5):
        super(AnnealedGumbelSoftmaxPolicy, self).__init__()
        self.model = PolicyNetwork(action_dim, weightspath, num_layers, bidirectional, freeze)
...
class ReplayBuffer:
    ...
    def sample(self, batch_size, double, device):
        ...
        h0 = torch.from_numpy(np.stack(h0, axis=0)).squeeze(2).permute(1,0,2).contiguous()
        c0 = torch.from_numpy(np.stack(c0, axis=0)).squeeze(2).permute(1,0,2).contiguous()
        h0_next = torch.from_numpy(np.stack(h0_next, axis=0)).squeeze(2).permute(1,0,2).contiguous()
        c0_next = torch.from_numpy(np.stack(c0_next, axis=0)).squeeze(2).permute(1,0,2).contiguous()
...
def run_experiment(args):
    ...
    actor = AnnealedGumbelSoftmaxPolicy(args.action_dim, args.weightspath, args.lstm_num_layers, args.lstm_bidirectional,\
                                        not args.unfreeze_policy, args.init_temperature, args.min_temperature,\
                                        args.max_temperature, args.temperature_anneal_rate)
    
    critic = CriticNetwork(args.action_dim, args.weightspath, args.lstm_num_layers, args.lstm_bidirectional,\
                           not args.unfreeze_critic)
    target_actor = AnnealedGumbelSoftmaxPolicy(args.action_dim, args.weightspath, args.lstm_num_layers, args.lstm_bidirectional,\
                                               not args.unfreeze_policy, args.init_temperature, args.min_temperature,\
                                               args.max_temperature, args.temperature_anneal_rate)
    target_critic = CriticNetwork(args.action_dim, args.weightspath, args.lstm_num_layers, args.lstm_bidirectional,\
                                  not args.unfreeze_critic)
...
def initialize_experiment():
    ...
    parser.add_argument("--lstm_bidirectional", type=str, default='True')
    ...
    args.lstm_bidirectional = True if args.lstm_bidirectional == 'True' else False
    ...
    settings = f"Configuration {args.rootname} ->\n\
        ...
        lstm_num_layers:{args.lstm_num_layers}, lstm_bidirectional:{args.lstm_bidirectional}\n\n\
    ...
    writepath = f'./experiments/td3_conv_pretrained_lstm2/{args.mazepath.split("/")[-1].split(".pkl")[0].split("_")[-1]}/{args.rootname}'
...
def load_model(path, device):
    ...
    model = AnnealedGumbelSoftmaxPolicy(checkpoint['args'].action_dim, checkpoint['args'].weightspath, checkpoint['args'].lstm_num_layers, checkpoint['args'].lstm_bidirectional,\
                                        not checkpoint['args'].unfreeze_policy, checkpoint['args'].init_temperature, checkpoint['args'].min_temperature,\
                                        checkpoint['args'].max_temperature, checkpoint['args'].temperature_anneal_rate)
...
def evaluate_agent():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weightspath', default='./experiments/td3_conv_pretrained_lstm2/Xa2BVk5Xko4VNHIr/X0q0EzBh/weights.pth', type=str, help='Path to pretrained weights.')
    parser.add_argument('--window_size', default=256, type=int)
