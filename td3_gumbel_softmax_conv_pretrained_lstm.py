...
class AnnealedGumbelSoftmaxPolicy(nn.Module):
    ...
    def forward(self, state, h0, c0, temperature=None):
        logits, h0_out, c0_out = self.policy_net(state, h0, c0)
...
def update_critic(args, critic, target_critic, batch, gamma, critic_optimizer, target_actor, temperature=None):
...
def update_actor(args, actor, critic, actor_optimizer, replay_buffer, batch_size, double, device):
...
def initiate_experiment(args):
    ...
    actor = AnnealedGumbelSoftmaxPolicy(args.action_dim, args.weightspath, not args.unfreeze_policy, args.init_temperature,\
                                        args.min_exploration_temperature, args.temperature_anneal_rate)
    ...
    if args.double:
        h0_actor, c0_actor = actor.policy_net.initialize_lstm_states(1, args.device, torch.DoubleTensor)
        # h0_target, c0_target = target_actor.policy_net.initialize_lstm_states(args.batch_size, args.device, torch.DoubleTensor)
    else:
        h0_actor, c0_actor = actor.policy_net.initialize_lstm_states(1, args.device, torch.FloatTensor)
        # h0_target, c0_target = target_actor.policy_net.initialize_lstm_states(args.batch_size, args.device, torch.FloatTensor)
    ...
    for step in range(args.max_steps):
        ...
        if step < args.exploration_steps:
            ...
            update_critic(args, critic, target_critic, batch, args.gamma, critic_optimizer, target_actor, args.policy_temperature)

            if step % args.policy_freq == 0:
                update_actor(args, actor,critic, actor_optimizer, replay_buffer, sample_size, args.double, args.device)
            ...
            if step % args.eval_steps == 0:
                ...
                while len(reward_cache) < args.episode_avg:
                    ...
                    if args.double:
                        h0_actor_eval, c0_actor_eval = actor.policy_net.initialize_lstm_states(1, args.device, torch.DoubleTensor)
                    else:
                        h0_actor_eval, c0_actor_eval = actor.policy_net.initialize_lstm_states(1, args.device, torch.FloatTensor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exploration_steps", type=int, default=1)
    ...
    parser.add_argument("--min_exploration_temperature", type=float, default=0.2)
    ...
    settings = f"Configuration {args.rootname} ->\n\
        ...
        init_temperature:{args.init_temperature}, min_exploration_temperature:{args.min_exploration_temperature}, policy_temperature:{args.policy_temperature}\n\
